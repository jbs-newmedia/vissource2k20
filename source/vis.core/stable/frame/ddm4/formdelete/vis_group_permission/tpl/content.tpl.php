...
if (!function_exists('vis_manager_group_permission')) {
	function vis_manager_group_permission($object, $element, $navigation_element, $ar_permission, $ro=false) {
		...
		<div style="padding-left:<?php echo(($navigation_element['info']['navigation_level'])*20) ?>px">
			...
			<?php if (count($navigation_element['links'])>0): ?><?php foreach ($navigation_element['links'] as $_navigation_element): ?><?php vis_manager_group_permission($object, $element, $_navigation_element, $ar_permission, $ro) ?><?php endforeach ?><?php endif ?>
		</div>
		<?php
	}
}
...
<div class="form-group ddm_element_<?php echo $this->getDeleteElementValue($element, 'id') ?>">
	...
	<?php foreach (\VIS\Core\Manager::getNavigationReal(0, $this->getGroupOption('navigation_level'), $this->getDeleteElementOption($element, 'tool_id')) as $navigation_element): ?><?php echo vis_manager_group_permission($this, $element, $navigation_element, $ar_permission, true) ?><?php endforeach ?>
