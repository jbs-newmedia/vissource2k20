...
if ($this->getEditElementOption($element, 'manager')===true) {
	$this->setEditElementStorage($element, \VIS\Core\Manager::loadUserMandantenByUserId($this->getIndexElementStorage(), 0));
} else {
	$this->setEditElementStorage($element, \VIS\Core\Manager::loadUserMandantenByUserId($this->getIndexElementStorage(), $this->getEditElementOption($element, 'tool_id')));
}
...
if (\osWFrame\Core\Settings::getAction()=='doedit') {
	...
	if ($this->getEditElementOption($element, 'manager')===true) {
		...
		foreach (\VIS\Core\Manager::getMandantenByToolId($this->getEditElementOption($element, 'tool_id'), true) as $mandant_id=>$mandant_name) {
