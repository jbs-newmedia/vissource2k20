...
$this->setEditElementStorage($element, \VIS\Core\Manager::loadUserGroupByGroupId($this->getIndexElementStorage(), $this->getEditElementOption($element, 'tool_id')));

if (\osWFrame\Core\Settings::getAction()=='doedit') {
	$ar_tool_user=[];
	foreach (\VIS\Core\Manager::getUsers() as $user_id=>$user_name) {
...
$users=\VIS\Core\Manager::getUsers();
