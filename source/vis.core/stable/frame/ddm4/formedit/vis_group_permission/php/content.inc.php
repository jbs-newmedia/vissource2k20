...
$this->setEditElementStorage($element, \VIS\Core\Manager::loadGroupPermission($this->getIndexElementStorage()));

if (\osWFrame\Core\Settings::getAction()=='doedit') {
	$ar_permission=[];
	foreach (\VIS\Core\Manager::getNavigationRealUnsorted($this->getEditElementOption($element, 'tool_id')) as $navigation_element) {
