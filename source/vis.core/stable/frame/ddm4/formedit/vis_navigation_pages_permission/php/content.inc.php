...
$this->setEditElementStorage($element, \VIS\Core\Manager::loadPagePermission($this->getIndexElementStorage(), $this->getEditElementOption($element, 'tool_id')));

if (\osWFrame\Core\Settings::getAction()=='doedit') {
	$ar_navigation_permission=[];
	foreach (\VIS\Core\Manager::getPermissionTextList($this->getEditElementOption($element, 'tool_id')) as $permission_flag=>$permission_name) {
