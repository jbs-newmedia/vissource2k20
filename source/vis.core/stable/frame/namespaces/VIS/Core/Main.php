...
class Main {
	...
	public function setEnvironment(Template $Template):self {
		...
		if (Resource::existsResource('VIS', $name)!==true) {
			$files=['js'.DIRECTORY_SEPARATOR.'vis.js', 'css'.DIRECTORY_SEPARATOR.'vis.css', 'img'.DIRECTORY_SEPARATOR.'profile.png'];

			Resource::copyResourcePath('modules'.DIRECTORY_SEPARATOR.'vis'.DIRECTORY_SEPARATOR, $dir.DIRECTORY_SEPARATOR.$version.DIRECTORY_SEPARATOR, $files);
			Resource::writeResource('VIS', $name, time());
		}
	...
	public function getResourceLink(string $file):string {
		...
		$lfile=Settings::getStringVar('settings_abspath').'modules'.DIRECTORY_SEPARATOR.'vis'.DIRECTORY_SEPARATOR.$file;

		if ((Filesystem::existsFile($rfile)!==true)||((Filesystem::getFileModTime($rfile))<(Filesystem::getFileModTime($lfile)))) {
			Resource::copyResourcePath('modules'.DIRECTORY_SEPARATOR.'vis'.DIRECTORY_SEPARATOR, $dir.DIRECTORY_SEPARATOR.$version.DIRECTORY_SEPARATOR, [$file]);
		}
	...
	public function setTool(string $tool):bool {
		...
		if ($this->validateTool($tool)===true) {
			$this->tool=$tool;
			Session::setStringVar('vis_tool', $this->tool);
	...
	public function loadTools():self {
		$this->tools==[];
		$this->tools[Settings::getStringVar('vis_login_module')]='Logon';
		$this->tools[Settings::getStringVar('vis_chtool_module')]='ChTool';
