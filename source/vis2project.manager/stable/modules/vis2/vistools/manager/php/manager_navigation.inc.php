...
$ar_tools=\VIS2\Core\Manager::getTools();
...
if (($tool_id!==null)&&(isset($ar_tools[$tool_id]))) {
	...
	$tool_details=\VIS2\Core\Manager::getToolDetails($_POST['tool_id']);

	$navigation_links[0]=['navigation_id'=>0, 'module'=>$osW_DDM4->getDirectModule(), 'parameter'=>'vistool='.$VIS2_Main->getTool().'&vispage='.$VIS2_Navigation->getPage(), 'text'=>$tool_details['tool_name']];
	...
	$navigation_links[0]=['navigation_id'=>0, 'module'=>$osW_DDM4->getDirectModule(), 'parameter'=>'vistool='.$VIS2_Main->getTool().'&vispage='.$VIS2_Navigation->getPage(), 'text'=>'Tool wählen'];
}
...
if (in_array($ddm_navigation_id, [0])) {
	...
	*/
	...
	$ddm4_elements['send']['tool_id']['options']['data']=\VIS2\Core\Manager::getTools();
...
if (in_array($ddm_navigation_id, [3])) {
	...
	$ar_level=[];
	$ar_data[0]='-';
	foreach (\VIS2\Core\Manager::getNavigationReal(0, $osW_DDM4->getGroupOption('navigation_level'), $_POST['tool_id']) as $navigation_element_1) {
	...
	$ddm4_elements['data']['page_id']['options']['data']=\VIS2\Core\Manager::getPagesByToolId($_POST['tool_id']);
