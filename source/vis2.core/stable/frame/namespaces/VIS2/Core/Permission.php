...
class Permission {
	...
	public function loadPermission():self {
		...
		foreach ($VIS2_Groups->getGroups() as $group_id=>$group_details) {
			...
			$QloadGroupPermission->prepare('SELECT * FROM :table_vis2_group_permission: AS gp INNER JOIN :table_vis2_page: AS p ON (p.page_name_intern=gp.permission_page) INNER JOIN :table_vis2_page_permission: AS pp ON (pp.permission_flag=gp.permission_flag AND pp.page_id=p.page_id) INNER JOIN :table_vis2_permission: AS pe ON (pe.permission_flag=pp.permission_flag) WHERE pe.permission_ispublic=:permission_ispublic: AND gp.group_id=:group_id:');
