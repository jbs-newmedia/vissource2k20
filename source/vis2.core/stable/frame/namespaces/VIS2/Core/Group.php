...
class Group {
	...
	public function loadGroups():self {
		...
		$QloadGroupData->prepare('SELECT * FROM :table_vis2_user_group: AS ug INNER JOIN :table_vis2_group: AS g ON (g.group_id=ug.group_id) WHERE g.group_ispublic=:group_ispublic: AND ug.tool_id=:tool_id: AND ug.user_id=:user_id:');
