...
<div class="form-group ddm_element_<?php echo $this->getDeleteElementValue($element, 'id') ?>">
	...
	<?php if ($this->getDeleteElementOption($element, 'manager')===true): ?>
		...
		<?php foreach (\VIS2\Core\Manager::getTools() as $tool_id=>$tool_name): ?>
			...
			<?php if ($tool_details['tool_use_mandant']=='1'): ?>
				...
				<?php foreach (\VIS2\Core\Manager::getMandantenByToolId($tool_id, true) as $group_id=>$group_name): ?>
	...
	<?php else: ?>
		...
		<?php foreach (\VIS2\Core\Manager::getMandantenByToolId($this->getDeleteElementOption($element, 'tool_id'), true) as $group_id=>$group_name): ?>
