...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($element, 'id') ?>">
	...
	<?php if ($this->getEditElementOption($element, 'read_only')===true): ?>
		...
		<?php else: ?>
			...
			<?php foreach (\VIS2\Core\Manager::getMandantenByToolId($this->getEditElementOption($element, 'tool_id'), true) as $mandant_id=>$mandant_name): ?>
	...
	<?php else: ?>
				...
				<?php if ($tool_details['tool_use_mandant']=='1'): ?>
					...
					<?php foreach (\VIS2\Core\Manager::getMandantenByToolId($tool_id, true) as $mandant_id=>$mandant_name): ?>
						...
						<div class="custom-control custom-checkbox">
							<?php echo $this->getTemplate()->Form()->drawCheckBoxField($element.'_'.$tool_id.'_'.$mandant_id, '1', ((isset($ar_user_mandant[$tool_id])&&(isset($ar_user_mandant[$tool_id][$mandant_id]))&&($ar_user_mandant[$tool_id][$mandant_id]==1))?1:0), ['input_parameter'=>'title="'.\osWFrame\Core\HTML::outputString($mandant_name).'"', 'input_class'=>'custom-control-input']) ?>
							<label class="custom-control-label<?php if ($this->getTemplate()->Form()->getErrorMessage($element)): ?> text-danger<?php endif ?>" for="<?php echo $element.'_'.$tool_id.'_'.$mandant_id ?>0"><?php echo \osWFrame\Core\HTML::outputString($mandant_name) ?></label>
						</div>
		...
		<?php else: ?>
			...
			<?php foreach (\VIS2\Core\Manager::getMandantenByToolId($this->getEditElementOption($element, 'tool_id'), true) as $mandant_id=>$mandant_name): ?>
				...
				<div class="custom-control custom-checkbox">
					<?php echo $this->getTemplate()->Form()->drawCheckBoxField($element.'_'.$this->getEditElementOption($element, 'tool_id').'_'.$mandant_id, '1', ((isset($ar_user_mandant[$this->getEditElementOption($element, 'tool_id')])&&(isset($ar_user_mandant[$this->getEditElementOption($element, 'tool_id')][$mandant_id]))&&($ar_user_mandant[$this->getEditElementOption($element, 'tool_id')][$mandant_id]==1))?1:0), ['input_parameter'=>'title="'.\osWFrame\Core\HTML::outputString($mandant_name).'"', 'input_class'=>'custom-control-input']) ?>
					<label class="custom-control-label<?php if ($this->getTemplate()->Form()->getErrorMessage($element)): ?> text-danger<?php endif ?>" for="<?php echo $element.'_'.$this->getEditElementOption($element, 'tool_id').'_'.$mandant_id ?>0"><?php echo \osWFrame\Core\HTML::outputString($mandant_name) ?></label>
				</div>
