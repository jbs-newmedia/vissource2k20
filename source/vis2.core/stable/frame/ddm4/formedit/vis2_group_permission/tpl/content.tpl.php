...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($element, 'id') ?>">
	...
	<?php if ($this->getEditElementOption($element, 'read_only')===true): ?>
		...
		<?php /* read only */ ?><?php $ar_permission=$this->getEditElementStorage($element) ?><?php foreach (\VIS2\Core\Manager::getNavigationReal(0, $this->getGroupOption('navigation_level'), $this->getEditElementOption($element, 'tool_id')) as $navigation_element): ?><?php echo vis2_manager_group_permission($this, $element, $navigation_element, $ar_permission, true) ?><?php endforeach ?>

	<?php else: ?>

		<?php /* input */ ?><?php $ar_permission=$this->getEditElementStorage($element) ?><?php foreach (\VIS2\Core\Manager::getNavigationReal(0, $this->getGroupOption('navigation_level'), $this->getEditElementOption($element, 'tool_id')) as $navigation_element): ?><?php echo vis2_manager_group_permission($this, $element, $navigation_element, $ar_permission) ?><?php endforeach ?>
