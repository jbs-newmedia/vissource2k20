...
$this->setEditElementStorage($element, \VIS2\Core\Manager::loadGroupPermission($this->getIndexElementStorage()));

if (\osWFrame\Core\Settings::getAction()=='doedit') {
	$ar_permission=[];
	foreach (\VIS2\Core\Manager::getNavigationRealUnsorted($this->getEditElementOption($element, 'tool_id')) as $navigation_element) {
	...
	$this->setDoEditElementStorage($element, $ar_permission);
}
