...
if ($this->getEditElementOption($element, 'manager')===true) {
	$this->setEditElementStorage($element, \VIS2\Core\Manager::loadUserGroupByUserId($this->getIndexElementStorage(), 0));
} else {
	$this->setEditElementStorage($element, \VIS2\Core\Manager::loadUserGroupByUserId($this->getIndexElementStorage(), $this->getEditElementOption($element, 'tool_id')));
}
...
if (\osWFrame\Core\Settings::getAction()=='doedit') {
	...
	if ($this->getEditElementOption($element, 'manager')===true) {
		...
		foreach (\VIS2\Core\Manager::getGroupsByToolId($this->getEditElementOption($element, 'tool_id')) as $group_id=>$group_name) {
			if ((isset($_POST[$element.'_'.$group_id]))&&($_POST[$element.'_'.$group_id]==1)) {
