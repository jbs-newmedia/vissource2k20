...
if ($this->getListElementOption($element, 'display_create_time')==true) {
	$_columns[$options['options']['prefix'].'create_time']=['name'=>$options['options']['prefix'].'create_time', 'order'=>(isset($_order[$options['name']]))?true:false, 'search'=>false,];
}

if ($this->getListElementOption($element, 'display_create_user')==true) {
	$_columns[$options['options']['prefix'].'create_user_id']=['name'=>$options['options']['prefix'].'create_user_id', 'order'=>(isset($_order[$options['name']]))?true:false, 'search'=>false,];
}

if ($this->getListElementOption($element, 'display_update_time')==true) {
	$_columns[$options['options']['prefix'].'update_time']=['name'=>$options['options']['prefix'].'update_time', 'order'=>(isset($_order[$options['name']]))?true:false, 'search'=>false,];
}

if ($this->getListElementOption($element, 'display_update_user')==true) {
	$_columns[$options['options']['prefix'].'update_user_id']=['name'=>$options['options']['prefix'].'update_user_id', 'order'=>(isset($_order[$options['name']]))?true:false, 'search'=>false,];
}
