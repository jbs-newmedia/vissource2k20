...
<div class="form-group ddm_element_<?php echo $this->getAddElementValue($element, 'id') ?>">
		...
		<?php else: ?>
			...
			<div class="custom-checkbox">
				<?php if (isset($ar_tool_user[$this->getAddElementOption($element, 'tool_id')])&&($ar_tool_user[$this->getAddElementOption($element, 'tool_id')]==1)): ?>
					...
					<?php echo '#1# '.\osWFrame\Core\HTML::outputString($this->getAddElementOption($element, 'tool_name')) ?>
				...
				<?php else: ?>
					...
					<?php echo '#0# '.\osWFrame\Core\HTML::outputString($this->getAddElementOption($element, 'tool_name')) ?>
	...
	<?php else: ?>
		...
		<?php else:?>
			...
			<div class="custom-control custom-checkbox">
				<?php echo $this->getTemplate()->Form()->drawCheckBoxField($element.'_'.$this->getAddElementOption($element, 'tool_id'), '1', ((isset($ar_tool_user[$this->getAddElementOption($element, 'tool_id')])&&($ar_tool_user[$this->getAddElementOption($element, 'tool_id')]==1))?1:0), ['input_parameter'=>'title="'.\osWFrame\Core\HTML::outputString($this->getAddElementOption($element, 'tool_name')).'"', 'input_class'=>'custom-control-input']) ?>
				<label class="custom-control-label<?php if ($this->getTemplate()->Form()->getErrorMessage($element)): ?> text-danger<?php endif ?>" for="<?php echo $element.'_'.$this->getAddElementOption($element, 'tool_id') ?>0"><?php echo \osWFrame\Core\HTML::outputString($this->getAddElementOption($element, 'tool_name')) ?></label>
			</div>
