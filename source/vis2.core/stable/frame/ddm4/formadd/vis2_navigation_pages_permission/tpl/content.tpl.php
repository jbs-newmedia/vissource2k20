...
<div class="form-group ddm_element_<?php echo $this->getAddElementValue($element, 'id') ?>">
	...
	<?php if ($this->getAddElementOption($element, 'read_only')===true): ?>
		...
		<?php $permission_list=\VIS2\Core\Manager::getPermissionTextList($this->getAddElementOption($element, 'tool_id')) ?>
		...
		<?php if (!empty($permission_list)): ?>
			...
			<?php foreach ($permission_list as $permission_flag=>$permission_text): ?>
				...
				<div class="custom-checkbox">
					<?php if (isset($ar_navigation_permission[$permission_flag])&&($ar_navigation_permission[$permission_flag]==1)): ?><?php echo $this->getGroupMessage('log_char_true').' '.\osWFrame\Core\HTML::outputString($permission_text) ?><?php else: ?><?php echo $this->getGroupMessage('log_char_false').' '.\osWFrame\Core\HTML::outputString($permission_text) ?><?php endif ?><?php echo $this->getTemplate()->Form()->drawHiddenField($element.'_'.$permission_flag, $ar_navigation_permission[$permission_flag]) ?>
				</div>
		...
		<?php if ($permission_list!==[]): ?>
			...
			<?php foreach ($permission_list as $permission_flag=>$permission_text): ?>
				...
				<div class="custom-control custom-checkbox">
					...
					<label class="custom-control-label<?php if ($this->getTemplate()->Form()->getErrorMessage($element)!==null): ?> text-danger<?php endif ?>" for="<?php echo $element.'_'.$permission_flag ?>0"><?php echo \osWFrame\Core\HTML::outputString($permission_text) ?></label>
				</div>
