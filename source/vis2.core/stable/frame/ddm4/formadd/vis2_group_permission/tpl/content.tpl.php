				...
				<?php foreach ($navigation_element['info']['permission'] as $flag): ?>
					...
					<?php else: ?>
						<div class="custom-control custom-checkbox">
							...
							<label class="custom-control-label<?php if ($object->getTemplate()->Form()->getErrorMessage($element)!==null): ?> text-danger<?php endif ?>" for="<?php echo 'page_'.$navigation_element['info']['page_name_intern'].'_'.$flag ?>0"><?php echo \osWFrame\Core\HTML::outputString(\VIS2\Core\Manager::getPermissionText($flag, $object->getAddElementOption($element, 'tool_id'))) ?></label>
						</div>
					<?php endif ?>
...
<div class="form-group ddm_element_<?php echo $this->getAddElementValue($element, 'id') ?>">
	...
	<?php if ($this->getAddElementOption($element, 'read_only')===true): ?>
		...
		<?php foreach (\VIS2\Core\Manager::getNavigationReal(0, $this->getGroupOption('navigation_level'), $this->getAddElementOption($element, 'tool_id')) as $navigation_element): ?>

			<?php echo vis2_manager_group_permission($this, $element, $navigation_element, $ar_permission, true) ?>
	...
	<?php else: ?>
		...
		<?php foreach (\VIS2\Core\Manager::getNavigationReal(0, $this->getGroupOption('navigation_level'), $this->getAddElementOption($element, 'tool_id')) as $navigation_element): ?>
			...
			<?php echo vis2_manager_group_permission($this, $element, $navigation_element, $ar_permission) ?>
