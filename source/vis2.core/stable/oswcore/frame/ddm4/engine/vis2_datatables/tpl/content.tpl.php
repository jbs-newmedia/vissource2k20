...
if (in_array(Settings::getAction(), ['search', 'add', 'edit', 'delete', '', 'log'], true)) {
    $this->addCSSCodeHead(
        '
.page-wrapper-modal {
	width:100%;
...
'
    ...
    echo $this->getTemplate()->Form()->startForm('form_search', 'current', $this->getDirectParameters(), [
...
'
    ...
    echo '<div class="page-wrapper-modal px-2 py-1">';
    echo $this->getTemplate()->Form()->startForm('form_add', 'current', $this->getDirectParameters(), [
        'form_parameter' => 'enctype="multipart/form-data"',
    ]);
...
'
    ...
    if ($this->setLock(\osWFrame\Core\Settings::catchStringValue($this->getGroupOption('index', 'database')), $this->getGroupOption('index', 'database'), $this->getGroupOption('user_id', 'data'), $this->getGroupOption('connection_lock', 'database')) !== true) {
        echo '<div class="alert alert-danger" role="alert" style="margin:15px 0px;">' . \osWFrame\Core\StringFunctions::parseTextWithVars($this->getGroupMessage('lock_error'), [
            'user' => \VIS2\Core\Manager::getUsernameById($this->getLockUserId(\osWFrame\Core\Settings::catchStringValue($this->getGroupOption('index', 'database')), $this->getGroupOption('index', 'database'), $this->getGroupOption('connection_lock', 'database'))),
        ]) . '</div>';
    }
    echo $this->getTemplate()->Form()->startForm('form_edit', 'current', $this->getDirectParameters(), [
        'form_parameter' => 'enctype="multipart/form-data"',
    ]);
...
'
    ...
    echo '<div class="page-wrapper-modal px-2 py-1">';
    echo $this->getTemplate()->Form()->startForm('form_delete', 'current', $this->getDirectParameters(), [
        'form_parameter' => 'enctype="multipart/form-data"',
    ]);
...
if (in_array(Settings::getAction(), ['', 'log'], true)) {
    if (Settings::getAction() !== '') {
        $this->setName($this->getName() . '_' . Settings::getAction());
        echo '<div class="page-wrapper-modal px-2 py-1">';
    } else {
    ...
    <div class="modal fade overflow-hidden pe-0 pb-4" id="ddm4modal_dialog_<?php echo $this->getName() ?>"
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="justify-content-end">
                        <a type="button" class="resize" onclick="resizeDDM4Modal()"
                           aria-label="<?php echo $this->getGroupMessage('form_maximize') ?>"><i
                                class="fa fa-2x fa-fw fa-window-maximize text-muted" aria-hidden="true"></i></a>
                        <a type="button" data-bs-dismiss="modal" data-bs-dismiss="modal"
                           aria-label="<?php echo $this->getGroupMessage('form_close') ?>"><i
                                class="fa fa-2x fa-fw fa-window-close text-muted" aria-hidden="true"></i></a>
                    </div>
                </div>
                <div class="modal-body" style="overflow:hidden; overflow-y: hidden;"><p></p></div>
                <div class="modal-footer">
                    <button name="ddm4_button_close" type="button" class="btn btn-secondary ddm4_btn_close"
                            data-bs-dismiss="modal"><?php echo $this->getGroupMessage('form_close') ?></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade overflow-hidden pe-0 pb-4" id="ddm4_controller_<?php echo $this->getName() ?>"
         data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
         aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button onclick="submitDDM4Modal_<?php echo $this->getName() ?>();" name="ddm4_button_submit"
                            type="button"
                            class="btn btn-primary ddm4_btn_search"><?php echo $this->getGroupMessage(
                                'form_search'
                            ) ?></button>
                    <button onclick="submitDDM4Modal_<?php echo $this->getName() ?>();" name="ddm4_button_submit"
                            type="button"
                            class="btn btn-primary ddm4_btn_add"><?php echo $this->getGroupMessage(
                                'form_add'
                            ) ?></button>
                    <button onclick="submitDDM4Modal_<?php echo $this->getName() ?>();" name="ddm4_button_submit"
                            type="button"
                            class="btn btn-primary ddm4_btn_edit"><?php echo $this->getGroupMessage(
                                'form_edit'
                            ) ?></button>
                    <button onclick="submitDDM4Modal_<?php echo $this->getName() ?>(true);" name="ddm4_button_delete"
                            type="button"
                            class="btn btn-danger ddm4_btn_delete"><?php echo $this->getGroupMessage(
                                'form_delete'
                            ) ?></button>
                    <button onclick="resetDDM4Modal_<?php echo $this->getName() ?>();" name="ddm4_button_reset"
                            type="button"
                            class="btn btn-secondary ddm4_btn_reset"><?php echo $this->getGroupMessage(
                                'form_reset'
                            ) ?></button>
                    <button name="ddm4_button_close" type="button" class="btn btn-secondary ddm4_btn_close"
                            data-bs-dismiss="modal"><?php echo $this->getGroupMessage('form_close') ?></button>
                    <button name="ddm4_button_cancel" type="button" class="btn btn-secondary ddm4_btn_cancel"
                            data-bs-dismiss="modal"><?php echo $this->getGroupMessage('form_cancel') ?></button>
                </div>
            </div>
        </div>
    </div>
