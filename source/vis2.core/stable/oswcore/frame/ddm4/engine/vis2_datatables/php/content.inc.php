...
$index_key = '';
if (in_array(Settings::getAction(), ['log', 'log_ajax'], true)) {
    $titles = [];
    ...
    $index_value = Settings::catchIntValue($index_key, 0, 'gp');
    ...
    $filter = $this->getGroupOption('filter', 'database');
    ...
    $ddm4_elements['data']['log_key']['options']['search'] = true;
...
switch (Settings::getAction()) {
    ...
    case 'edit':
        if ($this->getCounter('edit_elements') > 0) {
            Settings::setAction('edit');
            $_POST['modal'] = 1;
        } else {
            Settings::setAction('');
        }
        if ($this->setLock(
            Settings::catchStringValue($this->getGroupOption('index', 'database')),
            $this->getGroupOption('index', 'database'),
            $this->getGroupOption('user_id', 'data'),
            $this->getGroupOption('connection_lock', 'database')
        ) !== true
        ) {
            foreach ($this->getEditElements() as $element => $element_details) {
	...
	$(function() {
		...
		window.parent.vis2_notify("' . addslashes(\osWFrame\Core\StringFunctions::parseTextWithVars($this->getGroupMessage('lock_error'), [
                    'user' => $this->getLockUserId(\osWFrame\Core\Settings::catchStringValue($this->getGroupOption('index', 'database')), $this->getGroupOption('index', 'database'), $this->getGroupOption('connection_lock', 'database')),
                ])) . '", "danger");
	});
	'
            );
    ...
    case 'dolock':
        ...
        break;
    case 'log':
        Settings::setAction('log');

        break;
    case 'log_ajax':
        Settings::setAction('log_ajax');

        break;
    case 'ajax':
        Settings::setAction('ajax');
...
// Search
if ((Settings::getAction() === 'search') || (Settings::getAction() === 'dosearch')) {
    $load = false;
            ...
            foreach ($element_details['name_array'] as $_name) {
                if ($element_details['options']['prefix'] !== '') {
                    if (isset($result[$element_details['options']['prefix'] . $_name])) {
                        $load = true;
                        $this->setSearchElementStorage(
                            $element_details['options']['prefix'] . $_name,
                            $result[$element_details['options']['prefix'] . $_name]
                        );
                    }
                } else {
    ...
    if (Settings::getAction() === 'dosearch') {
        ...
        if ($this->getTemplate()->Form()->hasErrorMessages() === true) {
            ...
            if (Settings::catchValue('ddm4_search_delete', '0', 'p') === '0') {
                foreach ($this->getSearchElements() as $element => $element_details) {
                    if ((isset($element_details['name'])) && ($element_details['name'] !== '')) {
                    ...
                    if ((isset($element_details['name_array'])) && ($element_details['name_array'] !== [])) {
                        foreach ($element_details['name_array'] as $_name) {
                            if ($element_details['options']['prefix'] !== '') {
                                $data[$element_details['options']['prefix'] . $_name] = $this->getSearchElementStorage(
                                    $element_details['options']['prefix'] . $_name
                                );
                            } else {
...
// Add
if ((Settings::getAction() === 'add') || (Settings::getAction() === 'doadd')) {
    $this->setIndexElementStorage('');
	...
	$(function() {
        ...
        osWFrame\Core\SessionMessageStack::addMessage('session', 'danger', [
            'msg' => $this->getGroupMessage('add_load_error_title'),
        ]);
        $this->getTemplate()->addJSCodeHead(
            '
	$(function() {
...
if ((Settings::getAction() === 'edit') || (Settings::getAction() === 'doedit')) {
    $this->setIndexElementStorage(Settings::catchValue($this->getGroupOption('index', 'database'), '', 'pg'));
            ...
            foreach ($filter as $logic => $elements) {
                foreach ($elements as $element) {
                    $ar_values[] = $this->getGroupOption(
                        'alias',
                        'database'
                    ) . '.' . $element['key'] . $element['operator'] . $element['value'];
                }
            }
    ...
    $QloadData = $this::getConnection($this->getGroupOption('connection', 'database'));
    ...
    if ($this->getGroupOption('db_index_type', 'database') === 'string') {
        ...
        $QloadData->bindInt(':value_index:', (int)$this->getIndexElementStorage());
    }
    ...
    if ($QloadData->exec() === 1) {
        ...
        osWFrame\Core\SessionMessageStack::addMessage('session', 'danger', [
            'msg' => $this->getGroupMessage('edit_load_error_title'),
        ]);
        $this->getTemplate()->addJSCodeHead(
            '
	$(function() {
...
if ((Settings::getAction() === 'delete') || (Settings::getAction() === 'dodelete')) {
    ...
    if ($QloadData->exec() === 1) {
        ...
        osWFrame\Core\SessionMessageStack::addMessage('session', 'danger', [
            'msg' => $this->getGroupMessage('delete_load_error_title'),
        ]);
        $this->getTemplate()->addJSCodeHead(
            '
	$(function() {
    ...
    if (Settings::getAction() === 'dodelete') {
        if (Settings::catchValue('btn_ddm_cancel', '', 'p') !== '') {
            Settings::setAction('');
            $_POST = [];
        }
...
$_search = [];
if (in_array(Settings::getAction(), ['', 'ajax', 'log', 'log_ajax'], true)) {
    $vars = [];
    $_order = [];
    $_search = [];
    $_hidden = [];
    ...
    foreach ($_elements as $element => $options) {
        ...
        if ((isset($options['options'])) && (isset($options['options']['search'])) && ($options['options']['search'] === true)) {
            $file = Settings::getStringVar(
                'settings_abspath'
            ) . 'oswproject' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'search_pre.inc.php';
            $file_core = Settings::getStringVar(
                'settings_abspath'
            ) . 'oswcore' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'search_pre.inc.php';
            if (file_exists($file)) {
                include $file;
...
// ajax
if (in_array(Settings::getAction(), ['ajax', 'log_ajax'], true)) {
    // get parameters
    $columns = Settings::catchValue('columns', '', 'p');
    $order = Settings::catchValue('order', '', 'p');
    $search = Settings::catchValue('search', '', 'p');
    ...
    if (($order !== []) && ($order !== '')) {
        foreach ($order as $key => $values) {
            if ((isset($columns[$order[$key]['column']])) && (in_array(
                $columns[$order[$key]['column']]['data'],
                $vars,
                true
            ))
            ) {
                if (isset(
                    $ddm_order_case_array[$this->getGroupOption(
                        'alias',
                        'database'
                    ) . '.' . $columns[$order[$key]['column']]['data']]
                )
                    ...
                    $sql = '';
                    $sql .= 'CASE ' . $this->getGroupOption(
                        'alias',
                        'database'
                    ) . '.' . $columns[$order[$key]['column']]['data'] . ' ';
                    $i = 0;
                    if ($order[$key]['dir'] === 'asc') {
                        foreach ($ddm_order_case_array[$this->getGroupOption(
                            'alias',
                            'database'
                        ) . '.' . $columns[$order[$key]['column']]['data']] as $k => $v) {
                            $i++;
                            $sql .= 'WHEN ' . $this::getConnection(
                                $this->getGroupOption('connection', 'database')
                            )->escapteString($k) . ' THEN ' . $i . ' ';
                        }
                    } else {
                    ...
                    $database_order_array[] = $this->getGroupOption(
                        'alias',
                        'database'
                    ) . '.' . $columns[$order[$key]['column']]['data'] . ' ' . $order[$key]['dir'];
                }
            }
        }
    }
    ...
    if ((is_array($search) && (isset($search['value'])) && ($search['value'] !== '')) && ($_search !== [])) {
        foreach ($_search as $key) {
            if (isset($_elements[$key])) {
                $options = $_elements[$key];
                $file = Settings::getStringVar(
                    'settings_abspath'
                ) . 'oswproject' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'search.inc.php';
                $file_core = Settings::getStringVar(
                    'settings_abspath'
                ) . 'oswcore' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'search.inc.php';
                if (file_exists($file)) {
                    include $file;
                } elseif (file_exists($file_core)) {
                    include $file_core;
    ...
    foreach ($this->getSearchElements() as $element => $options) {
        if ((isset($options['validation'])) && (isset($options['validation']['module']))) {
            $file = Settings::getStringVar(
                'settings_abspath'
            ) . 'oswproject' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'search_ext.inc.php';
            $file_core = Settings::getStringVar(
                'settings_abspath'
            ) . 'oswcore' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'search_ext.inc.php';
            if (file_exists($file)) {
                include $file;
            } elseif (file_exists($file_core)) {
                include $file_core;
    ...
    $QgetDataLimit = $this::getConnection($this->getGroupOption('connection', 'database'));
    $QgetDataLimit->prepare('SELECT :index: FROM :table: AS :alias: WHERE :search_filter:');
    $QgetDataLimit->bindRaw(
        ':index:',
        $this->getGroupOption('alias', 'database') . '.' . $this->getGroupOption('index', 'database')
    );
    ...
    $this->clearStorageViewData();

    // load list
    $QgetData = $this::getConnection($this->getGroupOption('connection', 'database'));
    ...
    $QgetData->bindLimit(
        $this->getGroupOption('alias', 'database') . '.' . $this->getGroupOption('index', 'database'),
        osWFrame\Core\Settings::catchIntValue('length', 1, 'gp'),
        ((Settings::catchIntValue('start', 0, 'gp') / osWFrame\Core\Settings::catchIntValue('length', 1, 'gp')) + 1),
        'draw'
    );
    foreach ($QgetData->query() as $view_data) {
        ...
        foreach ($this->getListElements() as $element => $options) {
            $file = Settings::getStringVar(
                'settings_abspath'
            ) . 'oswproject' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'tpl' . \DIRECTORY_SEPARATOR . 'content.tpl.php';
            $file_core = Settings::getStringVar(
                'settings_abspath'
            ) . 'oswcore' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'list' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'tpl' . \DIRECTORY_SEPARATOR . 'content.tpl.php';
            if (file_exists($file)) {
                include $file;
            } elseif (file_exists($file_core)) {
                include $file_core;
        ...
        foreach ($view_data as $key => $value) {
            $view_data[$key] = (string) $value;
        }
        $this->addStorageViewData($view_data);
    }
    $limitrows = $QgetData->getLimitRows();
    $this->addParameter('ddm_page', $limitrows['current_page_number']);

    $this->setStorageViewLimitrows($limitrows);

    Network::dieJSON([
        'draw' => Settings::catchValue('draw', 1, 'gp'),
        'recordsTotal' => $datalimit,
        'recordsFiltered' => $limitrows['number_of_rows'],
        'data' => $this->getStorageViewData(),
    ]);
}

// list
if (in_array(Settings::getAction(), ['', 'log'], true)) {
    $_columns = [];
    ...
    if ($this->getPreViewElements() !== []) {
        foreach ($this->getPreViewElements() as $element => $options) {
            if (isset($options['module'])) {
                $file = Settings::getStringVar(
                    'settings_abspath'
                ) . 'oswproject' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'view' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'content.inc.php';
                $file_core = Settings::getStringVar(
                    'settings_abspath'
                ) . 'oswcore' . \DIRECTORY_SEPARATOR . 'frame' . \DIRECTORY_SEPARATOR . 'ddm4' . \DIRECTORY_SEPARATOR . 'view' . \DIRECTORY_SEPARATOR . $options['module'] . \DIRECTORY_SEPARATOR . 'php' . \DIRECTORY_SEPARATOR . 'content.inc.php';
                if (file_exists($file)) {
                    include $file;
                } elseif (file_exists($file_core)) {
                    include $file_core;
    ...
    $ajax = '$(document).ready(function() {
		ddm4datatables=$(\'#ddm4_datatables_' . $this->getName() . '\').DataTable({
			...
			"ajax": {
				"url": \'' . $this->getTemplate()->buildhrefLink(
        $this->getDirectModule(),
        $this->getDirectParameters(),
        false
    ) . '\',
				"data": {
					"action":\'' . $ajax . '\',
    ...
    $this->getTemplate()->addJSCodeHead(
        'function refreshDDM4Elements_' . $this->getName() . '(elem) {
		' . implode("\n", $this->getAjaxFunctions()) . '
	}'
    );

    $this->getTemplate()->addJSCodeHead(
        'function openDDM4Dialog_' . $this->getName() . '(elem) {
    ...
    if (in_array(Settings::getAction(), [''], true)) {
        if (in_array(Settings::catchStringValue('ao'), ['add', 'edit', 'delete', 'search'], true)) {
            $this->getTemplate()->addJSCodeHead(
                '$(document).ready(function() {
			openDDM4CoreModal_' . $this->getName() . '(\'' . $this->getTemplate()->buildhrefLink(
                    $this->getDirectModule(),
                    'action=' . Settings::catchStringValue('ao') . '&preload_id=' . Settings::catchStringValue(
                        'preload_id'
                    ) . '&modal=1&' . $this->getDirectParameters(),
                    false
                ) . '\', \'' . $this->getGroupOption('add_title', 'messages') . '\', \'add\', ' . $this->getCounter(
                    'add_elements'
                ) . ');
			});'
            );
