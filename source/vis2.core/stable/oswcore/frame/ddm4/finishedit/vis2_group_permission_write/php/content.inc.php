...
$permission_list = Manager::getPermissionTextList($this->getFinishElementOption($element, 'tool_id'));
    ...
    foreach ($permissions as $permission => $flag) {
        if (((!isset($ar_permission[$permission_page])) || (!isset($ar_permission[$permission_page][$permission]))) || ($ar_permission[$permission_page][$permission] !== $flag)) {
            if ($flag === 1) {
                    ...
                    if ($this->getFinishElementOption($element_storage, 'createupdatestatus_prefix') !== '') {
                        if (!in_array(
                            ...
                            DDM4_Log::addValue(
                                ...
                                '#0# ' . $ar_data[$permission_page] . ' : ' . $permission_list[$permission],
                                '#1# ' . $ar_data[$permission_page] . ' : ' . $permission_list[$permission],
                ...
                Manager::delGroupPermission($this->getIndexElementStorage(), $permission_page, $permission);
                if ($this->getGroupOption('enable_log') === true) {
                    if ($this->getFinishElementOption($element_storage, 'createupdatestatus_prefix') !== '') {
                        if (!in_array(
                            $element_current,
