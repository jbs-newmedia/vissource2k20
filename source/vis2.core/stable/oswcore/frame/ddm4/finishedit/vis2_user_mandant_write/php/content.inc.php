...
if ($this->getFinishElementOption($element, 'manager') === true) {
    foreach (Manager::getTools() as $tool_id => $tool_name) {
        $tool_mandants = Manager::getMandantenByToolId($tool_id, true);
            ...
            foreach ($ar_tool_user_do[$tool_id] as $mandant_id => $flag) {
                if (((!isset($ar_tool_user[$tool_id])) || (!isset($ar_tool_user[$tool_id][$mandant_id]))) || ($ar_tool_user[$tool_id][$mandant_id] !== $flag)) {
                    if ($flag === 1) {
                            ...
                            if ($this->getFinishElementOption($element_storage, 'createupdatestatus_prefix') !== '') {
                                if (!in_array(
                                    ...
                                    DDM4_Log::addValue(
                                        ...
                                        '#0# ' . $tool_name . ':' . $tool_mandants[$mandant_id],
                                        '#1# ' . $tool_name . ':' . $tool_mandants[$mandant_id],
                        ...
                        Manager::delUserMandant($this->getIndexElementStorage(), $mandant_id, $tool_id);
                        if ($this->getGroupOption('enable_log') === true) {
                            if ($this->getFinishElementOption($element_storage, 'createupdatestatus_prefix') !== '') {
                                if (!in_array(
                                    $element_current,
    ...
    $tool_mandants = Manager::getMandantenByToolId($this->getFinishElementOption($element, 'tool_id'), true);
