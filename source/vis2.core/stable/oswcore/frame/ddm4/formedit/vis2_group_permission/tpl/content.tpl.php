...
if (!function_exists('vis2_manager_group_permission')) {
    function vis2_manager_group_permission($object, $element, $navigation_element, $ar_permission, $ro = false): void
    {
        ...
        <div style="padding-left:<?php echo (($navigation_element['info']['navigation_level']) * 20) ?>px">
            ...
            <?php if (count(
                $navigation_element['info']['permission']
            ) > 0
            ): ?><?php foreach ($navigation_element['info']['permission'] as $flag): ?><?php if ($ro === true): ?>
                <div class="custom-checkbox">
                    <?php if (isset($ar_permission[$navigation_element['info']['page_name_intern']][$flag])): ?><?php echo $object->getGroupMessage(
                        'log_char_true'
                    ) . ' ' . HTML::outputString(
                        Manager::getPermissionText($flag, $object->getEditElementOption($element, 'tool_id'))
                    ) ?><?php else: ?><?php echo $object->getGroupMessage('log_char_false') . ' ' . HTML::outputString(
                        Manager::getPermissionText($flag, $object->getEditElementOption($element, 'tool_id'))
                    ) ?><?php endif ?><?php echo $object->getTemplate()->Form()->drawHiddenField(
            ...
            <?php if (count($navigation_element['links']) > 0): ?>
                ...
                <?php foreach ($navigation_element['links'] as $_navigation_element): ?>
                    ...
                    <?php vis2_manager_group_permission(
                        $object,
                        $element,
                        $_navigation_element,
                        $ar_permission,
...
<div class="form-group ddm_element_<?php echo $this->getEditElementValue($element, 'id') ?>">
    ...
    <?php else: ?>
        ...
        <?php /* input */ ?><?php $ar_permission = $this->getEditElementStorage(
            $element
        ) ?><?php foreach (Manager::getNavigationReal(
            0,
            $this->getGroupOption('navigation_level'),
