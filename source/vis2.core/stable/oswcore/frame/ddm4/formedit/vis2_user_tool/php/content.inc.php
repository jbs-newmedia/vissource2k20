...
if ($this->getEditElementOption($element, 'manager') === true) {
    $this->setEditElementStorage($element, Manager::loadUserTool($this->getIndexElementStorage(), 0));
} else {
    $this->setEditElementStorage(
        $element,
        Manager::loadUserTool($this->getIndexElementStorage(), $this->getEditElementOption($element, 'tool_id'))
    );
}
...
if (Settings::getAction() === 'doedit') {
    ...
    if ($this->getEditElementOption($element, 'manager') === true) {
        ...
        )) && ($_POST[$element . '_' . $this->getEditElementOption($element, 'tool_id')] === '1')
        ) {
