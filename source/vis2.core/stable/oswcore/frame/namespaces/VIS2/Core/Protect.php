...
class Protect
{
    ...
    public static function addEntry(int $user_id): bool
    {
        ...
        $QinsertData->prepare(
            'INSERT INTO :table_vis2_protect: (user_id, protect_create_time, protect_create_user_id, protect_update_time, protect_update_user_id) VALUES (:user_id:, :protect_create_time:, :protect_create_user_id:, :protect_update_time:, :protect_update_user_id:)'
        );
    ...
    public static function isBlocked(int $user_id): bool
    {
        ...
        if ($attempts < Settings::getIntVar('vis2_protect_attempts')) {
            return false;
        }

        if ($attempts >= Settings::getIntVar('vis2_protect_attempts_max')) {
            return true;
        }

        $attempts = bcdiv($attempts, Settings::getIntVar('vis2_protect_attempts'));
        $time = Settings::getIntVar('vis2_protect_time') * pow(2, $attempts - 1);

        $QgetData = self::getConnection();
        $QgetData->prepare(
            'SELECT protect_create_time FROM :table_vis2_protect: WHERE user_id=:user_id: ORDER BY protect_id ASC LIMIT :start:, 1'
        );
        $QgetData->bindTable(':table_vis2_protect:', 'vis2_protect');
        $QgetData->bindInt(':user_id:', $user_id);
        $QgetData->bindInt(':start:', ($attempts * Settings::getIntVar('vis2_protect_attempts')) - 1);
        $QgetData->execute();
        $QgetData->fetch();
