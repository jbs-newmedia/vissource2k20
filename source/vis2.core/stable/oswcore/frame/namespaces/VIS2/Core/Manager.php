...
class Manager
{
    ...
    protected static int $tool_id = 0;
    ...
    public static function setToolId(int $tool_id): void
    {
        self::$tool_id = $tool_id;
    }

    public static function getToolId(): int
    {
        return self::$tool_id;
    }
    ...
    public static function getToolDetails(int $tool_id = 0, bool $force = false): array
    {
        if ($tool_id === 0) {
            $tool_id = self::getToolId();
        }
    ...
    public static function getUsers(): array
    {
        if (self::$users === []) {
            $QselectUsers = self::getConnection();
            $QselectUsers->prepare(
                'SELECT * FROM :table_vis2_user: WHERE 1 ORDER BY user_lastname ASC, user_firstname ASC'
            );
    ...
    public static function getMandantenByToolId(int $tool_id = 0, bool $with_all = false, bool $force = false): array
    {
        ...
        if ((!isset(self::$mandantebytoolid[$tool_id])) || ($force === true)) {
            ...
            $QselectMandanten->prepare(
                'SELECT * FROM :table_vis2_mandant: WHERE tool_id=:tool_id: ORDER BY mandant_name ASC'
            );
    ...
    public static function getPagesByToolId(int $tool_id = 0, bool $force = false): array
    {
        ...
        if ((!isset(self::$pagesbytoolid[$tool_id])) || ($force === true)) {
            ...
            $QselectPages->prepare(
                'SELECT * FROM :table_vis2_page: WHERE tool_id=:tool_id: AND page_ispublic=:page_ispublic: ORDER BY page_name ASC, page_name_intern ASC'
            );
    ...
    public static function getPermissionTextList(int $tool_id = 0, bool $force = false): array
    {
        ...
        if ((!isset(self::$permissiontext[$tool_id])) || ($force === true)) {
            ...
            $Qselect->prepare(
                'SELECT * FROM :table_vis2_permission: WHERE tool_id=:tool_id: AND permission_ispublic=:permission_ispublic:'
            );
    ...
    public static function loadUserMandantenByMandantenId(int $mandant_id, int $tool_id): array
    {
        ...
        $QloadData = self::getConnection();
        $QloadData->prepare(
            'SELECT * FROM :table_vis2_user_mandant: WHERE mandant_id=:mandant_id: AND tool_id=:tool_id:'
        );
    ...
    public static function loadUserMandantenByUserId(int $user_id, int $tool_id): array
    {
        ...
        if ($tool_id === 0) {
            ...
            $QloadData->prepare(
                'SELECT * FROM :table_vis2_user_mandant: WHERE user_id=:user_id: AND tool_id=:tool_id:'
            );
        }
        $QloadData->bindTable(':table_vis2_user_mandant:', 'vis2_user_mandant');
        $QloadData->bindInt(':user_id:', $user_id);
    ...
    public static function addPagePermission(
        int $page_id,
        int $tool_id,
        ...
        $QinsertData->prepare(
            'INSERT INTO :table_vis2_page_permission: (page_id, tool_id, permission_flag, page_permission_create_time, page_permission_create_user_id, page_permission_update_time, page_permission_update_user_id) VALUES (:page_id:, :tool_id:, :permission_flag:, :page_permission_create_time:, :page_permission_create_user_id:, :page_permission_update_time:, :page_permission_update_user_id:)'
        );
    ...
    public static function delPagePermission(int $page_id, int $tool_id, string $permission_flag): bool
    {
        ...
        $QdeleteData->prepare(
            'DELETE FROM :table_vis2_page_permission: WHERE page_id=:page_id: AND tool_id=:tool_id: AND permission_flag=:permission_flag:'
        );
    ...
    public static function delPageById(int $page_id, int $tool_id): bool
    {
        /**
         * vis2_navigation
         */
    ...
    public static function delPermissionFlagByToolId(string $permission_flag, int $tool_id): bool
    {
        ...
        foreach ($QgetData->query() as $group) {
            ...
            $QdeleteData = self::getConnection();
            $QdeleteData->prepare(
                'DELETE FROM :table_vis2_group_permission: WHERE permission_flag=:permission_flag: AND group_id=:group_id:'
            );
        ...
        $QdeleteData = self::getConnection();
        $QdeleteData->prepare(
            'DELETE FROM :table_vis2_page_permission: WHERE permission_flag=:permission_flag: AND tool_id=:tool_id:'
        );
        $QdeleteData->bindTable(':table_vis2_page_permission:', 'vis2_page_permission');
        $QdeleteData->bindString(':permission_flag:', $permission_flag);
        $QdeleteData->bindInt(':tool_id:', $tool_id);
        ...
        $QdeleteData = self::getConnection();
        $QdeleteData->prepare(
            'DELETE FROM :table_vis2_permission: WHERE permission_flag=:permission_flag: AND tool_id=:tool_id:'
        );
    ...
    public static function getNavigationReal(int $parent_id = 0, int $max_level = 0, int $tool_id = 0): ?array
    {
        ...
        if (!isset(self::$navigation_real[$tool_id][$parent_id . '_' . $max_level])) {
            self::$navigation_real[$tool_id][$parent_id . '_' . $max_level] = self::createNavigationRealRecursive(
                $parent_id,
                0,
                $max_level,
                $tool_id
    ...
    public static function addUserTool(int $user_id, int $tool_id, int $create_time, int $create_user_id): bool
    {
        $QinsertData = self::getConnection();
        $QinsertData->prepare(
            'INSERT INTO :table_vis2_user_tool: (user_id, tool_id, user_tool_create_time, user_tool_create_user_id, user_tool_update_time, user_tool_update_user_id) VALUES (:user_id:, :tool_id:, :user_tool_create_time:, :user_tool_create_user_id:, :user_tool_update_time:, :user_tool_update_user_id:)'
        );
     ...
     *
     */
    public static function addUserGroup(
        int $user_id,
        int $group_id,
        int $tool_id,
        int $create_time,
        ...
        $QinsertData = self::getConnection();
        $QinsertData->prepare(
            'INSERT INTO :table_vis2_user_group: (group_id, tool_id, user_id, user_group_create_time, user_group_create_user_id, user_group_update_time, user_group_update_user_id) VALUES (:group_id:, :tool_id:, :user_id:, :user_group_create_time:, :user_group_create_user_id:, :user_group_update_time:, :user_group_update_user_id:)'
        );
    ...
    public static function delUserGroup(int $user_id, int $group_id, int $tool_id): bool
    {
        $QdeleteData = self::getConnection();
        $QdeleteData->prepare(
            'DELETE FROM :table_vis2_user_group: WHERE user_id=:user_id: AND tool_id=:tool_id: AND group_id=:group_id:'
        );
        $QdeleteData->bindTable(':table_vis2_user_group:', 'vis2_user_group');
        $QdeleteData->bindInt(':user_id:', $user_id);
        $QdeleteData->bindInt(':tool_id:', $tool_id);
     ...
     *
     */
    public static function addUserMandant(
        int $user_id,
        int $mandant_id,
        int $tool_id,
        int $create_time,
    ...
    public static function delUserMandant(int $user_id, int $mandant_id, int $tool_id): bool
    {
        $QdeleteData = self::getConnection();
        $QdeleteData->prepare(
            'DELETE FROM :table_vis2_user_mandant: WHERE user_id=:user_id: AND tool_id=:tool_id: AND mandant_id=:mandant_id:'
        );
    ...
    public static function delGroupPermission(int $group_id, string $permission_page, string $permission_flag): bool
    {
        $QdeleteData = self::getConnection();
        $QdeleteData->prepare(
            'DELETE FROM :table_vis2_group_permission: WHERE group_id=:group_id: AND permission_page=:permission_page: AND permission_flag=:permission_flag:'
        );
        $QdeleteData->bindTable(':table_vis2_group_permission:', 'vis2_group_permission');
        $QdeleteData->bindInt(':group_id:', $group_id);
    ...
    public static function delTool(int $tool_id): void
    {
        $QgetData = self::getConnection();
        $QgetData->prepare('SELECT group_id FROM :table_vis2_group: WHERE tool_id=:tool_id:');
        $QgetData->bindTable(':table_vis2_group:', 'vis2_group');
    ...
    protected static function createNavigationRealRecursive(
        int $parent_id,
        int $level,
        int $max_level,
        int $tool_id
        ...
        if (isset(self::$navigation_tree[$tool_id][$parent_id])) {
            foreach (self::$navigation_tree[$tool_id][$parent_id] as $category_id => $category) {
                ...
                if (isset(self::$navigation_tree[$tool_id][$category_id]) && (($max_level === 0) || ($max_level > $level + 1))) {
                    ...
                    $_data['info'] = $category;
                    $_data['links'] = self::createNavigationRealRecursive(
                        $category_id,
                        $level + 1,
                        $max_level,
                        $tool_id
