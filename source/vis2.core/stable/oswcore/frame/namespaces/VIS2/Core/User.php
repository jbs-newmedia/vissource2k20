...
class User
{
    ...
    {
        if ($this->getId() !== null) {
            if ((Settings::getBoolVar('vis2_protect_login_remember') !== true) || (Cookie::isCookiesEnabled(
            ) !== true)
            ) {
                $store_login = false;
            }
            ...
            $token_value = md5(
                $this->getEMail() . microtime() . uniqid(microtime()) . Settings::getStringVar('settings_protection_salt')
            );

            $QinsertData = self::getConnection();
            $QinsertData->prepare(
                'INSERT :table_vis2_user_token: (user_id, session_id, token_value, token_type, token_name, token_description, token_permanent, token_create_time, token_create_user_id, token_update_time, token_update_user_id) VALUES (:user_id:, :session_id:, :token_value:, :token_type:, :token_name:, :token_description:, :token_permanent:, :token_create_time:, :token_create_user_id:, :token_update_time:, :token_update_user_id:)'
            );
    ...
    public function createLoginByToken(string $user_token = ''): bool
    {
        ...
        $user_id = (int)(substr($user_token, 33));
        $token_value = substr($user_token, 0, 32);

        $QgetData = self::getConnection();
        $QgetData->prepare(
            'SELECT * FROM :table_vis2_user_token: AS ut LEFT JOIN :table_vis2_user: AS u ON (u.user_id=ut.user_id) WHERE ut.token_value=:user_token: AND ut.user_id=:user_id:'
        );
    ...
    public function setLoginCookieToken(string $user_token): self
    {
        ...
        if ($user_token !== '') {
            Cookie::setCookie(
                Settings::getStringVar('vis2_path') . '_user_token',
                $user_token,
                (time() + (60 * 60 * 24 * 365)),
                '/',
                $cookie_domain,
    ...
    {
        if ((Settings::catchStringCookieValue(
            Settings::getStringVar('vis2_path') . '_user_token'
        ) !== '') && (Settings::getBoolVar('vis2_protect_login_remember') === true)
        ) {
            return true;
        }
    ...
    public function doLogout(): bool
    {
        if ($this->getId() !== null) {
            ...
            $QdelData->prepare(
                'DELETE FROM :table_vis2_user_token: WHERE token_value=:token_value: AND user_id=:user_id:'
            );
    ...
    public function loadUserDetailsByToken(string $user_token): bool
    {
        ...
        $QselectUserDetails = self::getConnection();
        $QselectUserDetails->prepare(
            'SELECT * FROM :table_vis2_user_token: AS ut LEFT JOIN :table_vis2_user: AS u ON (u.user_id=ut.user_id) WHERE ut.token_value=:token_value: AND ut.user_id=:user_id:'
        );
        ...
        if ($QselectUserDetails->exec() === 1) {
            ...
            $QupdateToken->prepare(
                'UPDATE :table_vis2_user_token: SET token_update_time=:token_update_time:, token_update_user_id=:token_update_user_id: WHERE token_value=:token_value: AND user_id=:user_id:'
            );
    ...
    {
        $this->tools = [];
        $this->tools[Settings::getStringVar('vis2_login_module')] = [
            'tool_id' => 0,
            'tool_name' => 'Anmelden',
            'tool_name_intern' => Settings::getStringVar('vis2_login_module'),
        ];
        $this->tools[Settings::getStringVar('vis2_logout_module')] = [
            'tool_id' => 0,
            'tool_name' => 'Abmelden',
            'tool_name_intern' => Settings::getStringVar('vis2_logout_module'),
        ];
        $this->tools[Settings::getStringVar('vis2_chtool_module')] = [
            'tool_id' => 0,
        ...
        $QselectTools->prepare(
            'SELECT * FROM :table_vis2_tool: AS t INNER JOIN :table_vis2_user_tool: AS u ON (u.tool_id=t.tool_id) WHERE t.tool_ispublic=:tool_ispublic: AND u.user_id=:user_id: ORDER BY t.tool_name ASC'
        );
        ...
        foreach ($QselectTools->query() as $tool_details) {
            $this->tools[$tool_details['tool_name_intern']] = [
                'tool_id' => $tool_details['tool_id'],
                'tool_name' => $tool_details['tool_name'],
                'tool_name_intern' => $tool_details['tool_name_intern'],
            ];
    ...
    public function loadGroups(): self
    {
        ...
        $QselectGroups->prepare(
            'SELECT * FROM :table_vis2_group: AS g INNER JOIN :table_vis2_user_group: AS u ON (u.group_id=g.group_id) WHERE g.group_ispublic=:group_ispublic: AND u.user_id=:user_id: ORDER BY g.group_name ASC'
        );
        ...
        foreach ($QselectGroups->query() as $group_details) {
            $this->groups[$group_details['group_name_intern']] = [
                'group_id' => $group_details['group_id'],
                'group_name' => $group_details['group_name'],
                'group_name_intern' => $group_details['group_name_intern'],
            ];
    ...
    public function loadMandanten(): self
    {
        ...
        $QselectMandanten->prepare(
            'SELECT *, m.mandant_id as mandant_id FROM :table_vis2_mandant: AS m INNER JOIN :table_vis2_user_mandant: AS u ON ((u.mandant_id=m.mandant_id OR u.mandant_id=0 ) AND u.tool_id=m.tool_id) WHERE m.mandant_ispublic=:mandant_ispublic: AND u.user_id=:user_id: AND u.tool_id=:tool_id: ORDER BY m.mandant_name ASC'
        );
        ...
        foreach ($QselectMandanten->query() as $mandant_details) {
            $this->mandanten[$mandant_details['mandant_id']] = [
                'mandant_id' => $mandant_details['mandant_id'],
                'mandant_name' => $mandant_details['mandant_name'],
            ];
    ...
    {
        if ($full === true) {
            if (($this->getStringVar('user_form') !== null) && ($this->getStringVar(
                'user_firstname'
            ) !== null) && ($this->getStringVar('user_lastname') !== null)
            ) {
                return trim(
                    $this->getStringVar('user_form') . ' ' . $this->getStringVar('user_firstname') . ' ' . $this->getStringVar(
                        'user_lastname'
                    )
                );
            }
